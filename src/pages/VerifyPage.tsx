...
setNoteSendPayloadRaw(payloadRaw);
  ...
  const confirmNoteSend = useCallback(
    (override?: { meta?: NoteSendMeta | null; payload?: Record<string, unknown> | null }) => {
      ...
      const effectiveMeta =
        ...
        noteSendMeta ??
        (overridePayload
          ? buildNoteSendMetaFromObjectLoose(overridePayload)
          : noteSendPayloadRaw
            ? buildNoteSendMetaFromObjectLoose(noteSendPayloadRaw)
            : null);
      ...
      const effectivePayload = overridePayload ?? noteSendPayloadRaw;
      const transferLeafHash =
        effectiveMeta.transferLeafHashSend ??
        readRecordString(effectivePayload, "transferLeafHashSend") ??
        readRecordString(effectivePayload, "transferLeafHash") ??
        readRecordString(effectivePayload, "leafHash") ??
...
React.useEffect(() => {
  ...
  const onDownloadNotePng = useCallback(async () => {
    ...
    let downloadPayload: Record<string, unknown> | null = null;
    ...
    try {
      ...
      if (noteSendPayload) {
        downloadPayload = noteSendPayload as Record<string, unknown>;
        downloadMeta = buildNoteSendMetaFromObjectLoose(downloadPayload);
      }
      ...
      if (downloadPayload) {
        setNoteSendPayloadRaw(downloadPayload);
        setNoteSendMeta(downloadMeta);
      }
      // ✅ ALWAYS flip claim + force UI refresh (mobile-safe)
      confirmNoteSend(downloadPayload ? { payload: downloadPayload, meta: downloadMeta } : undefined);
    }
  }, [
    confirmNoteSend,
    noteClaimed,
