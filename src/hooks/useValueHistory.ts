// src/hooks/useValueHistory.ts (Kairos-only)
...
export function useValueHistory(opts?: Options) {
  ...
  // authoritative mutable buffer (NOT read during render)
  const bufRef = useRef<ValuePoint[]>([]);

  // render-safe snapshot (read during render)
  ...
  const pushSample = useCallback(
    (p: ValuePoint) => {
      ...
      // publish snapshot for rendering (new array)
      setSeries(buf.slice());
    },
    [maxPoints, maxBeats]
  );
