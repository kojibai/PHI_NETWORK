...
const fixturesRoot = new URL("./fixtures/sigils/", import.meta.url);
const readFixture = (name) => readFileSync(new URL(name, fixturesRoot), "utf8");
...
test("verifySigilSvg validates golden root fixture", async () => {
  const svg = readFixture("golden-root.svg");
  const slug = parseSlug("144-ROOT");
  const result = await verifySigilSvg(slug, svg);
  assert.equal(result.status, "ok");
  assert.equal(result.embedded?.pulse, 144);
});

test("verifySigilSvg validates golden desc fixture", async () => {
  const svg = readFixture("golden-desc.svg");
  const slug = parseSlug("288-DESC");
  const result = await verifySigilSvg(slug, svg);
  assert.equal(result.status, "ok");
  assert.equal(result.embedded?.pulse, 288);
});
